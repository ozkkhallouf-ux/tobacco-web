...
(function () {
  ...
  var SECRET_RULES = [
    // 1) ترويسة ترخيص — قاعدة واحدة تفرّق بين شكلَي القيمة.
    //
    // ⚠️ إصلاح ملاحظة Codex P1 الثانية على PR #188: القاعدة السابقة كانت
    // تنهي القيمة عند `\S+` أي عند أول فراغ، وقيمة Digest قائمة معاملات
    // مفصولة بفواصل: `username="ozk", realm="…", nonce="…", response="…"`.
    // فكان المحجوب `username="ozk",` وحده — وهو أقلّها حساسية — بينما
    // `response` (توقيع المصادقة نفسه) و`nonce` يمرّان إلى Rollbar سليمَين.
    // أنا من أضاف `digest` إلى تلك القائمة في الـcommit السابق، فأنشأتُ
    // الملاحظة بيدي: أُدرج المخطط بلا الانتباه إلى أن شكل قيمته مختلف.
    //
    // التفريق ضروري ولا يُغني عنه حجبٌ شامل حتى نهاية السطر دائماً: قيمة
    // Bearer/Basic رمز واحد، وما بعده على السطر نصّ تشخيصي مشروع
    // (`status=401`) لا يجوز ابتلاعه. أمّا Digest فقيمته السطر كلّه.
    //
    // وُحّدت القاعدتان في واحدة عمداً: تجربتان متتاليتان على النصّ نفسه
    // كانتا تُنتجان `Authorization: [سرّ محذوف] [سرّ محذوف]` — الثانية تلتقط
    // كلمة `Digest` التي خلّفتها الأولى فتحجب اسم المخطط أيضاً، فيضيع
    // «فشل مصادقة من نوع Digest» وهي معلومة تشخيصية بلا سرّ.
    //
    // `.` في JavaScript لا يطابق السطر الجديد، فالحجب يقف عند نهاية السطر
    // ولا يبتلع بقية أثر المكدّس. واشتراط الفاصل مباشرةً بعد الكلمة يمنع
    // مطابقة نصّ عادي مثل «Authorization failed: user lacks role».
    {
      re: /((?:proxy-)?authorization\s*[=:]\s*)(?:(bearer|basic|digest|negotiate)\s+)?(.*)/gi,
      to: function (match, prefix, scheme, value) {
        var head = prefix + (scheme ? scheme + " " : "");
        // Digest: قائمة معاملات — تُحجب حتى نهاية السطر بلا استثناء.
        if (scheme && scheme.toLowerCase() === "digest") return head + REDACTED;
        // غيره: رمز واحد يُحجب، وما بعده يبقى مقروءاً للتشخيص.
        return head + value.replace(/^\S+/, REDACTED);
      }
    },
