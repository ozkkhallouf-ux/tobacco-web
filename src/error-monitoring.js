...
(function () {
  ...
  var BUSINESS_SPACED_RE = new RegExp(
    "\\b(?:" + BUSINESS_FIELDS + "|credit limit)\\s+([-+]?\\s?[$\\u20AC\\u00A3\\u00A5\\u20BA\\uFDFC]?\\s?\\d[\\d.,]*)",
    "gi"
  );
  ...
  // ⚠️ إصلاح ملاحظة Codex P1 السادسة على PR #202: صون اللاحقة كان غير مشروط،
  // ولا حدّ لطول رقمَيها. فسطرٌ مثل `at invoice:1250000:987654` يُقبل إطاراً
  // (ينتهي بـ`:رقم:رقم`) وتُصان لاحقته حرفياً — فيمرّ رصيد ورقم فاتورة تحت
  // غطاء «سطر وعمود». أُثبت بالتشغيل.
  //
  // فاللاحقة تُصان الآن **فقط** إن كان رأس السطر مرجعَ مصدرٍ حقيقياً: فيه
  // مسار أو امتداد ملف شيفرة. و`at invoice` ليس فيه أيّهما، فيمرّ السطر كله
  // بقواعد الحجب. لا يُخسَر شيء: الإطار الحقيقي يحمل دائماً مسار ملفه.
  var SOURCE_HINT = /[\/\\]|\.(?:js|mjs|cjs|jsx|ts|tsx|html|css)\b/i;

  function redactFrame(line) {
    var m = line.match(FRAME_TAIL);
    if (m && SOURCE_HINT.test(m[1])) return redactBusinessData(m[1]) + m[2];
