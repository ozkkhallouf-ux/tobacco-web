(function initOzkPriceListTemplate(root) {
  ...
  // احتياطي انحراف الصفحة الأولى — وحده يفصلها عن عطل «الرأس وحده».
  //
  ...
  // طباعة مستقلّ. قياسٌ فعليّ للفارق بين المستندين على نفس البيانات: عمودٌ
  // قِيس 949px خرج مطبوعاً 902px — أي المستندان لا يتّفقان أصلاً، والاتجاه
  // ليس مضموناً (خطّ احتياطي أعرض لاسمٍ واحد يلفّه سطراً ثانياً فيطول العمود).
  //
  // وسلوك كروم عند التجاوز **ليس خطياً**، وهذا ما يحدّد المقاس. قياس فعلي عبر
  ...
  //  لا يغيّران هذا السلوك إطلاقاً، فالشطر ليس خياراً نملك فرضه.)
  //
  // إذاً النطاق الخطر ضيّق ومعروف: تجاوزٌ بين صفر و~8px. وبهامش الأمان وحده
  // (6px) كان أسوأ حالٍ مسموح يترك 5.58px فقط تحت حافة الورقة — أي **داخل**
  // النطاق الخطر مباشرةً، والعطل على بُعد خطأ تقريب واحد.
  //
  // وبعد تثبيت ارتفاع الصفحة على الورقة الحقيقية أعلاه يصير الخلوص معادلةً
  // بسيطة: **خلوص أسوأ حالٍ مسموح = الاحتياطي + هامش الأمان**. فالشرط أن يتجاوز
  // مجموعُهما النطاقَ الخطر (8px). والاحتياطي 6px يعطي 12px — أي مرّة ونصف
  // النطاق — فيُخرج أسوأ حال منه بفائض يستوعب انحراف التقريب.
  //
  // ولا نزيده أكثر: قياسٌ فعلي على بيانات بحجم الإنتاج (302 صنف) يُظهر عتبةً
  // حادّة. عند 6px تبقى النشرتان بثلاث أوراق وتبقى الصفحة الأولى بـ79 صفاً —
  // أي لا يتغيّر شكل النشرة إطلاقاً. ومن 8px فصاعداً تُدفع مجموعة خارج الصفحة
  // الأولى في نشرة الدولار فيفيض القسم الخاص إلى **ورقة رابعة** على كل نشرة.
  // وتلك الورقة لا تشتري أماناً إضافياً: احتياطيٌ أكبر لا يُلغي النطاق الخطر
  // بل يزحزحه فحسب، والحماية الحقيقية من الانحراف الكبير قائمة في مكانين آخرين
  // — أن الرأس لم يعد يُهجر أبداً (renderPagesBlock)، وأن إطار الطباعة لم يعد
  // يطبع قبل جهوزية الخطوط (printHtmlDocument في src/app.js).
  const FIRST_PAGE_DRIFT_RESERVE_PX = 6;
  ...
  function layoutGroupsMeasured(groups, heights, options = {}) {
    ...
    // الاحتياطي يُخصم من الصفحة الأولى وحدها: هي الوحيدة التي تتشارك ورقتها مع
    // الرأس المقاس في مستند آخر — راجع FIRST_PAGE_DRIFT_RESERVE_PX أعلاه. بقية
    // الصفحات تبدأ ورقتها بفاصل قسري فلا يشاركها شيء، وميزانيتها تبقى كاملة.
    const mainBudgets = {
      reducedFirstPageBudget: Math.max(0, effectivePageHeight - headerHeightPx - FIRST_PAGE_DRIFT_RESERVE_PX),
  ...
  //
  // **الرأس لا يُترك وحيداً على ورقة أبداً.** الصفحة الفارغة تقع حين لا تتّسع أي
  // مجموعة تحت الرأس. كانت تُحذف كتلتها الفارغة **مع إبقاء مكانها في المصفوفة**،
  // فيرث الفاصلَ القسري (`break-before:page`) أولُ بلوك مرسوم فعلاً — أي تُدفع
  // كل الأصناف إلى الورقة الثانية وتصل الزبون ورقةٌ أولى فيها الرأس وحده. وهو
  // العطل الإنتاجي المُبلَّغ في نشرة الليرة (صورة معاينة الطباعة: «4 أوراق»،
  // الأولى عنوان بلا صنف واحد).
  //
  // القاعدة الصحيحة: الفاصل القسري ملك أول كتلة **مرسومة**، لا أول كتلة مخطّطة.
  // فإن خرجت الصفحة الأولى بلا مجموعات، تشارك الكتلةُ التاليةُ الرأسَ على ورقته
  // بدل أن تهجره — والمتصفح يشطرها بين الورقتين إن لم تتّسع، فتبقى الورقة الأولى
  // حاملةً الرأس **ومعه أصناف**. ورقةُ عنوانٍ مستقلّة لا تُطبع إلا بقرار صريح،
  // لا كأثر جانبي لفاصلٍ ورثه بلوكٌ لم يكن صاحبه.
  function renderPagesBlock(pages, isDocumentFirstPage) {
    let renderedAny = false;
    return pages.map((page) => {
      if (!page.right.length && !page.left.length) return "";
      const forceBreak = !(isDocumentFirstPage && !renderedAny);
      renderedAny = true;
      return `
