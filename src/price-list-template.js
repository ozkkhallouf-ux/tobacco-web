(function initOzkPriceListTemplate(root) {
  ...
  // «نخلة» لم تصل الزبون إطلاقاً. الاشتقاق يجعل «اسم بلا وجهة» مستحيلاً بنيوياً.
  ...
  const CSS = `
    ...
    /* أنماط العرض المضغوط على الشاشات الضيقة — للقراءة على الهاتف فقط.
     *
     * السمة data-fixed-width تستثني **مسبار القياس** الذي يبني توزيع الصفحات
     * (buildMeasuredBulletinLayout في src/app.js). المسبار عرضه مثبَّت بـ794px،
     * لكن استعلامات الوسائط تُطابَق على عرض **النافذة** لا عرض العنصر: من نافذة
     * أضيق من 720px كان المسبار يقيس ارتفاعات المجموعات بمقاسات الهاتف المضغوطة،
     * فيحشر الحزّام مجموعات أكثر في العمود، ثم يُرسم مستند الطباعة بعرض 794px
     * (حيث لا ينطبق هذا الاستعلام) فيفيض المحتوى عن الورقة ويدفع كروم الفائض
     * إلى أوراق إضافية — عين عطل «صفحة ثالثة فارغة وصفحة رابعة بعد فراغ».
     * القياس يجب أن يجري بنفس شروط الطباعة دائماً، مهما كان عرض النافذة. */
    @media screen and (max-width:720px) {
      .ozk-price-list:not([data-fixed-width]) .price-list-columns { gap:4px; padding:0 4px 6px; }
      .ozk-price-list:not([data-fixed-width]).has-document-tools .price-list-header { padding-top:102px; }
      .ozk-price-list:not([data-fixed-width]) .price-list-document-tools { right:10px; left:10px; justify-content:center; }
      .ozk-price-list:not([data-fixed-width]) .price-list-group { margin-bottom:3px; }
      .ozk-price-list:not([data-fixed-width]) .price-list-group-header { padding:4px 5px; font-size:9px; }
      .ozk-price-list:not([data-fixed-width]) td { padding:3px 4px; font-size:8px; }
      .ozk-price-list:not([data-fixed-width]) td.name { width:55%; }
      .ozk-price-list:not([data-fixed-width]) td.unit { width:15%; font-size:7px; }
      .ozk-price-list:not([data-fixed-width]) td.price { width:30%; font-size:8px; }
    }
    @media print {
  ...
  // رئيسية نصف فارغة وتُطبع بعدها صفحة كاملة للفحم والمعسل — ورق مهدور بلا سبب.
  //
  // القاعدة المطبَّقة هنا: تنزل المجموعة إلى الفراغ المتبقي **فقط إذا اتّسعت
  // كاملةً** في عمودها. لا تُقسَّم مجموعة أبداً بين صفحتين ولا بين عمودين، ولا
  // يتغيّر ترتيب القراءة: نستهلك بادئة الطابور فقط (نتوقف عند أول مجموعة لا
  // تتّسع) كي لا تسبق مجموعةٌ متأخرةٌ مجموعةً أسبق منها.
  //
  // كل طابور يبقى في عموده الأصلي: مجموعات SPECIAL_RIGHT_GROUPS إلى عمود
  // اليمين وSPECIAL_LEFT_GROUPS إلى عمود اليسار — الهوية البصرية للقسم لا
  // تتغيّر لمجرد أنه نزل بصفحة سابقة. ولا نُعيد موازنة الأعمدة بعد الملء
  // (balancePageColumns) لأن الموازنة تنقل آخر مجموعة بين العمودين، وهو ما
  // يكسر هذه الهوية.
  //
  // ما يتبقّى بعد الملء يُحزَم بصفحات مستقلة كما كان تماماً؛ وإن نزل القسم
  // بأكمله فلا تُطبع صفحة خاصة إطلاقاً.
  function fillTrailingSpaceWithSpecialGroups(pages, budgets, specialRight, specialLeft, heights, safetyMarginPx) {
    ...
    // packGroupsIntoBalancedPages: العمود يسحب من طابوره أولاً، ثم يسمح بفيضان
    // الطابور الآخر **بعد نفاده هو**. بدون هذا الفيضان كان عمود اليسار يبقى
    // شبه فارغ (وينشستر + معسل فقط) بينما تُدفع فيبات وقداحات وسلفان إلى صفحة
    // مستقلة لا تحمل غير ~12% من الورقة — أي بقي الهدر الذي جاءت هذه الميزة
    // لإزالته. الترتيب محفوظ: كل طابور يُستهلك كبادئة متصلة لا كقطع متفرقة.
  ...
  function layoutGroups(groups) {
    ...
    // تختفي أصنافه من نشرة الزبون بلا أثر. المستهلكون يرفضون التصدير عند امتلائه.
