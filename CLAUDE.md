...
| `اسأل <أي سؤال عن العمل>` | سياق مبني من المبيعات/الأرصدة/المخزون + Grok (`XAI_API_KEY` في أسرار دالة `telegram-webhook`) | جواب حر بالعربية العامية السورية مبني على بيانات حقيقية فقط. الرسالة الصوتية تُفرَّغ أولاً بـ Whisper (`OPENAI_API_KEY`) ثم تدخل نفس المسار النصي |
...
- الرسالة الصوتية (زر الميكروفون في تيليغرام) تُفرَّغ بـ Whisper إلى نص عربي ثم تدخل نفس مسار الأوامر أو سؤال Grok. التسجيل نفسه يتم داخل تيليغرام؛ البوت لا يسجّل من المايكروفون مباشرة.
...
| قيمة سطر الفاتورة أو سعر وحدته منفوخان (كرتونة واحدة سعرها 286$ تُطبع 14,300$) | `lineTotal` القادم من الأمين يساوي `Price × Qty` و`Qty` بالكروز بينما `Price` سعر الكرتونة — فالقيمة «من الأمين» نفسها مبنية على وحدة خاطئة، والأساس قد يختلف من سطر لآخر بالفاتورة نفسها | الحكم بمطابقة مجموع الأسطر مع إجمالي الفاتورة (`invoiceLineBasisPlan` في `src/app.js`)؛ فاتورة مجموعها المخزَّن مطابق تبقى بلا مساس، وبلا توزيع مطابق لا حكم. راجع `docs/ai/topics/printing.md` |
...
## Serena usage policy — سياسة استخدام Serena (خاصة بمشروع tobacco-web)

Serena هي **أداة البحث والتنقل الأساسية** في هذا المستودع. التزم بما يلي:

### البحث والتنقل
- قبل أي `grep` أو بحث نصي واسع، استخدم Serena أولاً.
- استخدم `find_symbol` و`get_symbols_overview` لتحديد الرمز أو الدالة أو الملف المطلوب.
- استخدم `find_referencing_symbols` لفهم الأماكن التي تعتمد على الرمز **قبل** تعديله.
- استخدم `grep` أو القراءة الكاملة **فقط** إذا كانت المعلومة غير رمزية
  (HTML، CSS، SQL، PowerShell، ملفات workflows) أو إذا لم تعطِ Serena نتيجة كافية.

### القراءة
- لا تقرأ الملفات الكبيرة كاملة مثل `src/app.js` إلا عند الضرورة القصوى.
- اقرأ فقط الرمز أو الجزء المطلوب كلما أمكن.

### التعديل
- استخدم أدوات Serena الرمزية للتعديل (`replace_symbol_body`، `insert_after_symbol`،
  `insert_before_symbol`، `rename_symbol`) عندما تكون مناسبة.
- قبل أي تعديل، افهم العلاقات والتأثيرات الجانبية للرمز المستهدف.
- حافظ على **أقل نطاق تعديل ممكن**، وتجنّب Refactor واسع بدون طلب صريح.

### التحقق
- بعد أي تعديل، شغّل الاختبارات والفحوص المرتبطة بالجزء المعدَّل
  (`npm run check` كحد أدنى) وتأكد من عدم حدوث Regression.
