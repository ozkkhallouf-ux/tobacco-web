...
| الأرشفة إلى iCloud (ماك) | `tools/mac-archive-bridge/`, `src/icloud-archive.js`, نقاط الربط في `src/app.js`, `index.html` (CSP) | كسر تصدير/طباعة قائمة، توسيع CSP، كتابة خارج المجلد المقصود | `npm run check` + تصدير مستند فعلي من الماك + التأكد أن إطفاء الجسر لا يكسر التصدير |
...
6. إن كان التعديل إصلاحَ عطل إنتاجي حقيقي: اختبار انحدار يفشل قبل الإصلاح وينجح بعده — [سياسة اختبارات الانحدار](../regression-policy.md).
7. `npm run check:critical` إن مسّ التعديل الواجهة أو التنقّل أو الطباعة.
