...
async function bootPage(context, { items, prices, rate }) {
  ...
  await page.evaluate(({ items, prices, rate }) => {
    ...
    // `dataStore` في app.js مرجع ثابت إلى window.tobaccoData — نستبدل الدوال
    // على الكائن نفسه كي يراها الكود المُختبَر.
    const reports = [{ items, summary: { syncedAt: "2026-08-31T00:00:00.000Z" } }];

    // زرع الحالة وحده لا يكفي: مُحمِّلات بدء التشغيل (loadInventoryReports،
    // loadApprovedPrices) تعمل بالتوازي مع هذا الزرع، وكلاهما يكتب على نفس
    // الحقول. الأسرع يخسر. وأسوأ من ذلك أن فرع «Supabase مُهيّأ بلا جلسة»
    // يضبط الحقلين إلى [] صراحةً — فحين تستقر تلك الوعود **بعد** الزرع (وهو ما
    // يحدث على عدّاء CI الأبطأ) تُمسح بيانات الاختبار، فيفشل buildBulletinDataset
    // وتظهر شاشة الخطأ بلا زر تصدير: عين الفشل المتقطّع في فحوص الهاتف.
    // العلاج: نجعل المتجر نفسه مصدر هذه البيانات، فأي تحميل — الآن أو بعد ثوانٍ
    // من التحديث التلقائي — يُعيد تطبيق نفس التجهيزات بدل مسحها.
    window.__savedRates = [];
    window.__fixtureRate = rate;
    // isConfigured=false يُسقط حارس «مُهيّأ بلا جلسة» الذي يفرّغ الحقول قبل أن
    // يصل النداء إلى المتجر أصلاً؛ أثره الباقي نصّي بحت (صياغة تنبيهات).
    ...
    // ما يُحفظ هو ما يُقرأ: يمنع إعادة تحميل لاحقة من إرجاع سعر الصرف إلى قيمته
    // الأولى بعد أن غيّره الفحص عمداً (سيناريو A→B).
    window.tobaccoData.getSyriaExchangeRate = async () => window.__fixtureRate;
    window.tobaccoData.setSyriaExchangeRate = async (value) => {
      window.__savedRates.push(Number(value));
      window.__fixtureRate = Number(value);
      return Number(value);
    };
...
// ===== 6) أرشفة النشرة لا تسقط بإعادة الهيكلة =====
// المسار القديم على main يؤرشف النشرة باستدعاء archiveToICloud("price_list", ...)
// مباشرةً. إعادة الهيكلة تنقل الخطّاف إلى خيار `archive` في printHtmlDocument —
// القدرة نفسها بصيغة مختلفة. هذا الفحص يثبت **السلوك** (أن نوع المستند يصل
// فعلاً) فلا تسقط الأرشفة صامتة في أي إعادة هيكلة لاحقة.
{
  const context = await browser.newContext({ viewport: { width: 1440, height: 900 }, bypassCSP: true, serviceWorkers: "block" });
