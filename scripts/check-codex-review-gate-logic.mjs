...
// 8) إصلاح جوهري ثالث عشر بتاريخ 2026-08-31 — ردّاً على P1 حيّين رصدهما Codex على
//    commit dd260aba37 نفسه (فوق الإصلاح الثاني عشر): (أ) تثبيت الملكية حتى لحظة الكتابة
//    الفعلية في publish (لا يكفي الفوز اللحظي بالقفل وقت acquire_lock)، و(ب) عدم إسقاط
//    آخر متنافس بصمت — رفع MAX_ATTEMPTS + jitter عشوائي + تحذير ::warning:: مرئي عند
//    الاستنفاد الفعلي (نادر عملياً)، دون إفشال الـjob (يحافظ على نقاء statusCheckRollup).
assert.match(
  yml,
  /CURRENT_LOCK_SHA=\$\(gh api "repos\/\$REPO\/git\/ref\/\$LOCK_REF" --jq '\.object\.sha \/\/ empty' 2>\/dev\/null \|\| echo ""\)/,
  'خطوة publish يجب أن تعيد قراءة مرجع القفل الحيّ مباشرة قبل أي كتابة فعلية — الفوز اللحظي بالقفل وقت acquire_lock لا يضمن بقاء هذا التشغيل هو الحائز حتى لحظة الكتابة',
);
assert.match(
  yml,
  /if \[ "\$CURRENT_LOCK_GENERATION" != "\$GENERATION" \][\s\S]{0,400}exit 0/,
  'يجب مقارنة تامة (لا "أحدث أو يساوي" فقط) بين generation القفل الحيّ الآن وgeneration هذا التشغيل قبل الكتابة — أي عدم تطابق (تجاوزه تشغيل آخر) يوجب الخروج بلا كتابة',
);
assert.match(
  yml,
  /MAX_ATTEMPTS=20/,
  'يجب رفع سقف محاولات أخذ القفل إلى ٢٠ (كان ٦) لتقليل احتمال استنفاد المحاولات بسبب تنافس عابر بحت إلى ما يقارب الصفر عملياً',
);
assert.match(
  yml,
  /sleep "\$\(\( \(RANDOM % 3\) \+ 1 \)\)"/,
  'يجب استخدام تأخير عشوائي (jitter) بين محاولات إعادة أخذ القفل بدل تأخير ثابت — يقلّل تصادم المحاولات المتكررة تحت تنافس كثيف',
);
assert.match(
  yml,
  /::warning title=Codex Review Gate/,
  'عند استنفاد كل محاولات أخذ القفل بسبب تنافس عابر (لا staleness)، يجب إصدار تحذير ::warning:: مرئي في سجلّ التشغيل بدل الخروج الصامت — كي لا يضيع تقييم صحيح دون أثر مرئي لأحد يراجع السجلّات',
);
assert.doesNotMatch(
  yml,
