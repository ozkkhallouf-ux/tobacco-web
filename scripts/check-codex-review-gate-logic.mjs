...
// 8) إصلاح جوهري رابع عشر بتاريخ 2026-08-31 — ردّاً على P1 حيّين رصدهما Codex على
//    commit 780adea691 نفسه (فوق الإصلاح الثالث عشر): GitHub Checks API لا يدعم
//    compare-and-swap إطلاقاً، فأي فحص "تحقق ثم اكتب" أحادي الطلقة يبقى عرضة نظرياً لسباق.
//    (أ) publish يستبدل فحص lock-ref المُزال بحلقة write-then-verify تعتمد حصراً على
//    external_id للـcheck-run الكانوني نفسه، مع تأكيد فوري بعد كل كتابة. (ب) acquire_lock
//    وpublish كلاهما يستخدمان مهلة زمنية (180 ثانية) بدل عدّاد محاولات ثابت — لا استسلام
//    بسبب عدد محاولات تعسفي طالما لم تُرصد staleness حقيقية.
assert.doesNotMatch(
  yml,
  /CURRENT_LOCK_SHA=\$\(gh api "repos\/\$REPO\/git\/ref\/\$LOCK_REF" --jq '\.object\.sha \/\/ empty' 2>\/dev\/null \|\| echo ""\)/,
  'فحص lock-ref أحادي الطلقة في publish (الإصلاح الثالث عشر) يجب أن يكون قد أُزيل بالكامل — لا يكفي لإغلاق سباق الكتابة الفعلية؛ استُبدل بحلقة write-then-verify مبنية على external_id',
);
assert.match(
  yml,
  /PUBLISH_DEADLINE=\$\(\( \$\(date \+%s\) \+ 180 \)\)/,
  'خطوة publish يجب أن تستخدم مهلة زمنية (180 ثانية) لحلقة الكتابة-ثم-التأكيد بدل الاعتماد على فحص أحادي قبل الكتابة',
);
assert.match(
  yml,
...
assert.match(
  yml,
  /if \[ "\$CONFIRM_GENERATION" = "\$GENERATION" \][\s\S]{0,50}PUBLISHED="true"/,
  'النشر لا يُعتبر مؤكَّداً إلا عندما تطابق القراءة الفورية بعد الكتابة generation هذا التشغيل بالضبط',
);
assert.doesNotMatch(
  yml,
  /^\s*MAX_ATTEMPTS=20\s*$/m,
  'يجب إزالة تعريف متغيّر عدّاد المحاولات الثابت (MAX_ATTEMPTS=20) من الكود الفعلي — الإصلاح الرابع عشر يستبدله بمهلة زمنية كي لا يُسقَط آخر متنافس شرعي بصمت لمجرد استنفاد عدد محاولات تعسفي (ذِكر الرقم داخل تعليقات توثيق الإصلاحات السابقة مقبول تاريخياً)',
);
assert.match(
  yml,
  /LOCK_DEADLINE=\$\(\( \$\(date \+%s\) \+ 180 \)\)/,
  'خطوة acquire_lock يجب أن تستخدم مهلة زمنية (180 ثانية) بدل عدّاد محاولات ثابت لإعادة محاولة أخذ القفل',
);
assert.match(
  yml,
  /sleep "\$\(\( \(RANDOM % 3\) \+ 1 \)\)"/,
  'يجب استخدام تأخير عشوائي (jitter) بين محاولات إعادة أخذ القفل/النشر بدل تأخير ثابت — يقلّل تصادم المحاولات المتكررة تحت تنافس كثيف',
);
assert.match(
  yml,
  /::warning title=Codex Review Gate — استنفاد مهلة القفل::/,
  'عند استنفاد المهلة الزمنية لأخذ القفل بسبب تنافس عابر (لا staleness)، يجب إصدار تحذير ::warning:: مرئي في سجلّ التشغيل بدل الخروج الصامت',
);
assert.match(
  yml,
  /::warning title=Codex Review Gate — استنفاد مهلة النشر::/,
  'عند استنفاد المهلة الزمنية لتأكيد النشر (تنافس كتابة مستمر)، يجب إصدار تحذير ::warning:: مرئي منفصل بدل الخروج الصامت',
);
assert.doesNotMatch(
  yml,
