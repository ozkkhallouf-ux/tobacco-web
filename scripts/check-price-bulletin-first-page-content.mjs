...
// ===== ٢) العطل الحتمي: صفحة أولى مخطَّطة فارغة تهجر الرأس =====
// حين لا تتّسع أي مجموعة تحت الرأس تخرج `mainPages[0]` فارغة. الكتلة الفارغة
// لا تُرسم — وكان مكانها يبقى في المصفوفة فيرث **أولُ بلوك مرسوم** الفاصلَ
// القسري، فتُدفع كل الأصناف للورقة الثانية ويبقى الرأس وحيداً. هذا هو العطل
// المُبلَّغ حرفياً، ويُبنى هنا من نفس دالتَي التصدير (`render` + `printDocument`)
// اللتين يستدعيهما زرّ «حفظ / مشاركة PDF» في `exportBulletinPdf`.
{
  ...
  check("صفحة أولى مخطَّطة فارغة: الورقة الأولى تحمل الرأس",
    sheetCarriesHeader(firstSheet), "الرأس نفسه غاب عن الورقة الأولى");

  check("صفحة أولى مخطَّطة فارغة: **الورقة الأولى تحمل أصنافاً ولا تخرج ورقة عنوان**",
    onFirst > 0,
    `صفر صنف على الورقة الأولى من ${built.rows.length} — الرأس هُجر وحده`
    + ` (أول كتلة تحمل فاصلاً قسرياً: ${printed.geometry.blocks[0]?.forcedBreak})`);

  check("صفحة أولى مخطَّطة فارغة: أول كتلة مرسومة بلا فاصل قسري",
    printed.geometry.blocks[0]?.forcedBreak === false,
    "أولُ بلوك مرسوم ورث break-before:page من صفحة فارغة لم تُرسم أصلاً");

  const anywhere = built.rows.filter((row) => printed.sheets.some((sheet) => printedRow(sheet, row))).length;
  check("صفحة أولى مخطَّطة فارغة: لا صنف يضيع من الملف",
