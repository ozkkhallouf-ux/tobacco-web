...
--
-- ⚠️ هذه الترحيلة صارت **شرطاً تشغيلياً**، لا تحسيناً اختيارياً:
-- tools/push-supplier-obligations.ps1 لم يعد يملك مسار delete-then-insert
-- إطلاقاً — ينشر عبر هذه الدالة وحدها. فقبل تطبيق هذا الملف على الإنتاج يفشل
-- كل تشغيل للمنتج بخطأ صريح (PGRST202) بلا أن يكتب شيئاً، وسكريبت التسجيل
-- tools/register-supplier-obligations-task.ps1 يرفض جدولة المهمة أصلاً ما لم
-- يُمرَّر -AtomicReplacementApplied تأكيداً أن هذه الدالة حيّة.
--
-- العطل الذي أزالته: المسار القديم كان يحذف كل صفوف المصدر ثم يُدخل البديل.
-- بين الحذف والإدخال نافذة يكون فيها الجدول فارغاً، وأي انقطاع شبكة داخلها
-- يترك الالتزامات ممسوحة.
--
...
create or replace function public.replace_supplier_obligations(
  p_source text,
  p_rows jsonb,
  -- حين يسدّد آخر مورد دائن تكون الحمولة الفارغة **حقيقة محاسبية** لا عطلاً،
  -- والرفض المطلق كان سيُبقي دَيناً على من سدّد. المنتج على Windows يمرّر true
  -- هنا فقط حين تتحقّق القراءة المصدرية نفسها (أرجعت موردين مرتبطين بفواتير
  -- شراء وكلهم مسدَّدون) — أي أن الإذن مشتقّ من دليل لا من عَلَم سطر أوامر.
  -- والقراءة الفارغة تماماً تُجهض في المنتج قبل أن تصل إلى هنا أصلاً.
...
-- المنتج يستدعيها هكذا (نداء واحد لكل مصدر، بلا تقسيم دفعات — التقسيم يكسر
-- الذرّية لأن كل نداء يحذف ما ليس في دفعته هو):
--   POST /rest/v1/rpc/replace_supplier_obligations
--   { "p_source": "ameen_ac000_credit_minus_debit", "p_rows": [...],
--     "p_allow_empty": false }
--
-- ويُمرَّر p_allow_empty=true في حالتين اثنتين لا ثالثة لهما:
--   • الحالة النهائية المُتحقَّقة: القراءة أرجعت موردين وكلهم مسدَّدون.
--   • تقاعد المصدر القديم ameen_cu000_credit_minus_debit، وبعد نجاح كتابة
--     الجيل الحالي وحده — كي لا يُترك الجدول بلا أي التزام إن فشل ما سبق.
