...
--   disabled > stuck > failed > ok > inflight > never_run
-- 'failed' تسبق 'stuck' من حيث المبدأ (الفشل حقيقة مكتملة والجمود استنتاج
-- زمني) — إلا حين تكون المحاولة الجارية فوقه هي نفسها قد تجاوزت مهلة stuck
-- الآن (Codex P1، الجولة الثالثة على PR #220): عندها 'stuck' تسبق، لأنها
-- الحالة الحيّة الحقيقية الآن، لا 'failed' القديم الذي لم يعد يعكس واقع
-- المحاولة الجارية. راجع بند 8 (فشل يبقى failed لأن المحاولة لم تتجاوز
-- المهلة بعد) مقابل بند 10 (نفس الشكل لكن تجاوزت المهلة ⇒ stuck).
--
...
begin
  ...
  -- Codex P1 (الجولة الثالثة على PR #220): محاولة جارية فوق فشل نهائي *وقد
  -- تجاوزت هي نفسها مهلة stuck الآن* ليست "الفشل لا يُبتلع" (بند 8) بل حالة
  -- حيّة جديدة تستحق إنذارها المستقل — راجع استثناء 'failed' في الدالة.
  assert private.cron_job_health(true,'running',t0-interval '20 minutes','failed',t0-interval '25 minutes',g,t0) = 'stuck',
    '10: retry متجاوز مهلة stuck فوق فشل نهائي سابق ⇒ stuck (الحالة الحيّة الآن، لا failed القديم)'; n:=n+1;
